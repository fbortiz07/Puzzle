...
let urlImg            = "img/image03.jpg",
    ...
    matrizDesorganiza = [],
    valorMatriz = {}; //variable global para gtuardar las nuevas posiciones de las fichas...
...
let MoverFicha = (fila, columna, fila2, columna2) =>
{
  if(!compruebaPuzzle()){
...
let contador=0;
    for (let i = 0; i < valorMatriz.length; i++) {
        ...
        for (let c = 0; c < valorMatriz.length; c++) {
            if(valorMatriz[i][c].cont === matrizPuzzle[i][c].cont)
                contador++;
        };

    };
    if(contador === (matrizDesorganiza.length*matrizDesorganiza.length))
        return true;
...
utils.accesoDOM("comprueba").addEventListener('click', event =>
{
  if(compruebaPuzzle())
    alert("Haz Ganado");
        ...
        alert("Aun no haz completado el puzzle, sigue intentandolo");
});
