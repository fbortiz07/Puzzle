        ...
        for (var i in direcciones) {
            ...
            if (enPosicion) {
                ...
                if (!matrizDesorganiza[posBusca.fila][posBusca.columna].ocupado) {
                    MoverFicha(fila, columna, posBusca.fila, posBusca.columna);
                }
            }
        }
    }
...
var MoverFicha = function MoverFicha(fila, columna, fila2, columna2) {
    if (!compruebaPuzzle()) {

        matrizDesorganiza[fila2][columna2].ocupado = true;
        matrizDesorganiza[fila][columna].ocupado = false;
        valorMatriz[fila2][columna2].fila = valorMatriz[fila][columna].fila;
        valorMatriz[fila2][columna2].columna = valorMatriz[fila][columna].columna;
        valorMatriz[fila2][columna2].cont = valorMatriz[fila][columna].cont;

        imprimePuzzle(valorMatriz);
    }
};

var compruebaPuzzle = function compruebaPuzzle() {
    var contador = 0;
    for (var i = 0; i < valorMatriz.length; i++) {

        for (var c = 0; c < valorMatriz.length; c++) {
            if (valorMatriz[i][c].cont === matrizPuzzle[i][c].cont) contador++;
        };
    };
    if (contador === matrizDesorganiza.length * matrizDesorganiza.length) return true;else return false;
};
...
_utils2.default.accesoDOM("comprueba").addEventListener('click', function (event) {
    if (compruebaPuzzle()) alert("Haz Ganado");else alert("Aun no haz completado el puzzle, sigue intentandolo");
});
